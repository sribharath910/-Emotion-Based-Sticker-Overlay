...
face_mesh = mp_face_mesh.FaceMesh(
    static_image_mode=False, max_num_faces=3, min_detection_confidence=0.5
)
...
def overlay_sticker(frame, sticker, x, y, size=50, alpha=1.0):
    ...
    fg = cv2.bitwise_and(sticker_rgb, sticker_rgb, mask=mask_gray)
...
while cap.isOpened():
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            # Label per face
            cv2.putText(frame, f"{emotion}", (x_min, y_min - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0), 2)

    # FPS Counter
    curr_time = time.time()
    fps = 1 / (curr_time - prev_time) if prev_time else 0
    prev_time = curr_time
    cv2.putText(frame, f"FPS: {int(fps)}", (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)

    # Faces detected
    cv2.putText(frame, f"Faces: {faces_detected}", (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 200, 255), 2)
